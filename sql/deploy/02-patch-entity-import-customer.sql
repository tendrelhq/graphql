-- Deploy graphql:patch-entity-import-customer to pg

BEGIN;

-- The procedure that exists in test/beta has a 10min delay baked in, which
-- really fucks up our tests because we want to immediately import the newly
-- created customer. This is therefore a temporary patch to remove the baked in
-- delay in order for our tests to proceed in CI. Note that eventually the test
-- infrastructure will use the public customer CRUD API after which this patch
-- will no longer be necessary.

CREATE OR REPLACE PROCEDURE entity.import_entity_customer(
	IN intervaltype text)
LANGUAGE 'plpgsql'
AS $BODY$
Declare
