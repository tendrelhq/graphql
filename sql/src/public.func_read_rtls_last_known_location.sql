...
-- Type: FUNCTION ; Name: func_read_rtls_last_known_location(timestamp with time zone,text[],text[],text[]); Owner: bombadil
...
GRANT EXECUTE ON FUNCTION func_read_rtls_last_known_location(timestamp with time zone,text[],text[],text[]) TO bombadil WITH GRANT OPTION;
