...
describe.skip("/api/template_field", () => {
  test("create", async () => {
    const url = new URL("/template_field?select=id", baseurl);
  ...
  test("read", async () => {
    const url = new URL(
      "/template_field?select=id,default_value,display_name(value),type(id,display_name(value))&order=id",
      baseurl,
    );
    const response = await fetch(url, {
      method: "GET",
  ...
  test.skip("update", async () => {
    ...
    const url = new URL(
      `/template_field?id=eq.${template}&select=id,_order`,
      baseurl,
    );
    const response = await fetch(url, {
      method: "PATCH",
  ...
  test("delete", async () => {
    const template = "0ae109a5-9ea9-4d60-aa63-dee867901bed"; // FIXME: need a demo customer
    const url = new URL(
      `/template_field?id=eq.${template}&select=id,_deleted`,
      baseurl,
    );
    const response = await fetch(url, {
      method: "DELETE",
